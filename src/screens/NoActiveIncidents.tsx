...
export function NoActiveIncidents(props: NoActiveIncidentsProps) {
  return (
    <>
      ...
      <span className="font-headline-sm text-headline-sm font-bold text-primary truncate">HQ Alpha</span>
      <span className="font-label-md text-label-md text-on-surface-variant truncate">Sector 7 Command</span>
      </div>
      </div>
      {/* Navigation Links */}
      <nav className="flex flex-col gap-1 w-full flex-1">
      {/* Active Tab (Dashboard) */}
      <a className="flex items-center gap-3 px-3 h-touch-target bg-secondary-container text-on-secondary-container rounded-lg active:opacity-80 transition-opacity" href="#">
      <span className="material-symbols-outlined" style={{fontVariationSettings: "'FILL' 1"}}>dashboard</span>
      <span className="font-label-md text-label-md">Dashboard</span>
      </a>
      {/* Inactive Tab (Insights) */}
      <a className="flex items-center gap-3 px-3 h-touch-target text-on-surface-variant hover:text-on-surface hover:bg-surface-variant/50 transition-all rounded-lg active:opacity-80" href="#">
      <span className="material-symbols-outlined">monitoring</span>
      <span className="font-label-md text-label-md">Insights</span>
      </a>
      {/* Inactive Tab (Settings) */}
      <a className="flex items-center gap-3 px-3 h-touch-target text-on-surface-variant hover:text-on-surface hover:bg-surface-variant/50 transition-all rounded-lg active:opacity-80 mt-auto" href="#">
      ...
      <input className="bg-surface-container-low border border-outline-variant rounded-DEFAULT h-8 pl-9 pr-3 font-body-md text-body-md text-on-surface placeholder:text-on-surface-variant focus:outline-none focus:border-primary-container focus:ring-1 focus:ring-primary-container transition-colors w-48 lg:w-64" placeholder="Search..." type="text" />
      </div>
      {/* Trailing Icons */}
      <div className="flex items-center gap-1">
      <button className="h-8 w-8 flex items-center justify-center rounded-DEFAULT text-on-surface-variant hover:text-on-surface hover:bg-surface-container-high transition-colors active:scale-95">
      <span className="material-symbols-outlined text-[20px]">notifications</span>
      </button>
      <button className="h-8 w-8 flex items-center justify-center rounded-DEFAULT text-on-surface-variant hover:text-on-surface hover:bg-surface-container-high transition-colors active:scale-95 md:hidden">
      ...
      {/* Call to Action */}
      <button className="flex items-center justify-center gap-2 bg-primary-container text-on-primary-container font-label-md text-label-md px-6 py-3 min-w-[180px] h-touch-target rounded-DEFAULT hover:brightness-110 active:scale-95 transition-all outline-none focus-visible:ring-2 focus-visible:ring-primary-container focus-visible:ring-offset-2 focus-visible:ring-offset-background group shadow-[0_4px_14px_0_rgba(37,99,235,0.2)]">
