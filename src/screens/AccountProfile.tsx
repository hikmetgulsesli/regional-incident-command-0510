...
export function AccountProfile(props: AccountProfileProps) {
  return (
    <>
      ...
      <button aria-label="Close panel" className="w-touch-target h-touch-target flex items-center justify-center rounded text-on-surface-variant hover:bg-surface-variant hover:text-on-surface focus:outline-none focus:ring-2 focus:ring-primary-container focus:ring-offset-2 focus:ring-offset-surface transition-colors">
      ...
      <img alt="Profile Photo" className="w-full h-full object-cover" data-alt="A professional headshot of a female military or emergency management commander in a dark, tactical uniform. The lighting is dramatic and cinematic, reminiscent of a high-tech command center. The background is a blurred dark slate grey, conveying a serious, utilitarian, and authoritative mood suitable for a mission-critical dashboard profile." src="https://lh3.googleusercontent.com/aida-public/AB6AXuDwz6JQxazjaqig8SXx4Liw-AgPtfV_6qdmdaablKK0BiE9RqwhOlEn_T3mXyq3MuOP9oYtdzzZl5SDHkwSvrcBq7JFPvs5d9TrXOh12WqtnBDt-3U7r_X3O0IeqxkYlm-0sQpxN9vaPiN6YdhZhTfVctNkkO0P2ZBvN-Mjaljze-i5dSSG2ssvUPBBVW06PTLdDZziyi_Zxx-jir3UksMDRE_8-hAPahLhg-7SlBOdKFs5MF1-FB7tGRcmb3QMiDsk3MqSEf8Tx_c" />
      </div>
      <div className="flex flex-col gap-unit flex-1">
      <h1 className="font-headline-md text-headline-md text-on-surface">Cmdr. Jane Doe</h1>
      ...
      <span>Command Sector: HQ Alpha</span>
      </div>
      </div>
      </div>
      </section>
      ...
      <button aria-pressed="false" className="relative w-12 h-6 rounded-full bg-surface-variant border border-outline-variant focus:outline-none focus:ring-2 focus:ring-primary-container focus:ring-offset-2 focus:ring-offset-surface-container">
      <span className="absolute left-[2px] top-[2px] w-5 h-5 rounded-full bg-on-surface-variant transition-transform"></span>
      </button>
      </div>
      {/* Toggle Item 2 */}
      <div className="flex items-center justify-between p-gutter min-h-[56px]">
      ...
      <button aria-pressed="true" className="relative w-12 h-6 rounded-full bg-primary-container focus:outline-none focus:ring-2 focus:ring-primary-container focus:ring-offset-2 focus:ring-offset-surface-container">
      <span className="absolute right-[2px] top-[2px] w-5 h-5 rounded-full bg-on-primary-container transition-transform"></span>
      </button>
      </div>
      {/* Toggle Item 3 */}
      <div className="flex items-center justify-between p-gutter min-h-[56px]">
      ...
      <button className="w-full flex items-center justify-center gap-unit h-touch-target border border-outline-variant bg-transparent text-on-surface font-label-md text-label-md uppercase tracking-wider hover:bg-surface-variant focus:outline-none focus:ring-2 focus:ring-primary-container focus:ring-offset-2 focus:ring-offset-surface transition-colors">
